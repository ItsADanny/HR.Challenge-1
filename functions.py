...
def shop(curr_player):
    ...
    if Choice == "BUY":
        print(f"<{colorama.Fore.CYAN}{shopkeeper_fullname}{colorama.Style.RESET_ALL}> What would you like to buy?")
        parts_list = ""
        ...
        while not exit:
            ...
            if input_buy.upper in options:
                if input_buy.upper == "EXIT":
                    exit = True
                else:
                    curr_player.add_rocketparts(input_buy.upper)
                    curr_player.remove_money(2500)
                    print(f"<{colorama.Fore.CYAN}{shopkeeper_fullname}{colorama.Style.RESET_ALL}> Thanks for the sell! Here are your 1500 Imperial Credits")
            else:
                print(f"<{colorama.Fore.YELLOW}{curr_player.get_username()}{colorama.Style.RESET_ALL}> Whoops, i didn't use valid option. I am going to retry that")
        ...
        while not exit:
            ...
            if input_sell.upper in options:
                if input_sell.upper == "EXIT":
                    exit = True
                else:
                    curr_player.remove_rocketparts(input_sell.upper)
        ...
        while is_playing:
            ...
            for letter in guessed_letters_array:  # Check if letter is guessed
                if letter_input == letter:
                    ...
                    print(">You Alredy Guessed: " + letter)
                    break
...
def random_event(curr_player):
    ...
    >It seems fortune smiles upon you")
        ...
        if typeLoss == 1:  # loses rocketpart
            item_count = len(curr_player.get_rocketpart_list())  # check inventory
    ...
    >Luckily you don't own anything and you lose nothing")
                ...
                item_position = random.randrange(item_count)  # choses random item
                item_lost = curr_player.get_curr_rocketpart_list_pos(item_position)  # sets up the removal
